...
struct ApplyOnnxModelTask {
  ...
  void process(aod::Tracks const& tracks)
  {
    ...
    for (auto& track : tracks) {
      LOGF(INFO, "collision id: %d; eta: %.3f; phi: %.3f; x: %.3f, y: %.3f, z: %.3f",
           track.collisionId(), track.eta(), track.phiraw(), track.x(), track.y(), track.z());

      std::vector<float> input_tensor_values{track.eta(), track.phiraw(), track.x(), track.y(), track.z()};
